...
class Deduper:
    ...
    def check_md5(self):
        ...
        for md5 in cursor.fetchall():
            if md5[0] is not None:
                cursor = self.con.execute(
                    "SELECT path FROM Files WHERE md5 = ?", (md5)
                )
    ...
    def mk_missing_md5s(self, size):
        ...
        for file in cursor.fetchall():
            ...
            # print(f"!!!{file[0]}:{md5}")
            cursor.execute("UPDATE Files SET md5 = ? WHERE path = ?", (md5, file[0]))
        self.con.commit()
    ...
    def scan_cache(self, *, scan_dir):
        ...
        cursor = self.con.execute("SELECT path FROM Files WHERE path LIKE ?", (expr,))

        for path in cursor.fetchall():
            if not Path(path[0]).exists():
                cursor = self.con.execute("DELETE FROM Files WHERE path = ?", (path,))
        self.con.commit()
